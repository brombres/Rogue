...
- [API] Added `Value.values(list=null:Value)->Value`.  For tables, returns a ValueList of the table data.  For anything else (including the ValueList) returns the value itself.  If a ValueList parameter is provided, values will be added to the list and it will be returned instead of creating and returning a new list.
