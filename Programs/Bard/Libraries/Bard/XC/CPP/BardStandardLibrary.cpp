...
void BardXCInteger64__create__Integer_Integer( BardVM* vm );
void BardXCInteger64__high( BardVM* vm );
void BardXCInteger64__low( BardVM* vm );
void BardXCInteger64__operatorPLUS__Integer64_Integer64( BardVM* vm );
void BardXCInteger64__operatorMINUS__Integer64_Integer64( BardVM* vm );
void BardXCInteger64__operatorTIMES__Integer64_Integer64( BardVM* vm );
void BardXCInteger64__operatorDIVIDEDBY__Integer64_Integer64( BardVM* vm );
void BardXCInteger64__to_Real( BardVM* vm );
...
void BardStandardLibrary_configure( BardVM* vm )
{
  ...
  BardVM_register_native_method( vm, "Integer64", "create(Integer,Integer)", BardXCInteger64__create__Integer_Integer );
  BardVM_register_native_method( vm, "Integer64", "high()", BardXCInteger64__high );
  BardVM_register_native_method( vm, "Integer64", "low()", BardXCInteger64__low );
  BardVM_register_native_method( vm, "Integer64", "operator+(Integer64,Integer64)", BardXCInteger64__operatorPLUS__Integer64_Integer64 );
  BardVM_register_native_method( vm, "Integer64", "operator-(Integer64,Integer64)", BardXCInteger64__operatorMINUS__Integer64_Integer64 );
  BardVM_register_native_method( vm, "Integer64", "operator*(Integer64,Integer64)", BardXCInteger64__operatorTIMES__Integer64_Integer64 );
  BardVM_register_native_method( vm, "Integer64", "operator/(Integer64,Integer64)", BardXCInteger64__operatorDIVIDEDBY__Integer64_Integer64 );
  BardVM_register_native_method( vm, "Integer64", "to_Real()", BardXCInteger64__to_Real );
...
//  Console
//=============================================================================
BardXCCharacter BardConsole_last_character_printed = 10;
...
static void BardConsole_write_character_to_debug_output( BardXCCharacter ch )
{
...
void BardConsole__native_write__String( BardVM* vm )
{
  ...
  if (st)
  {
    ...
    if (count)
    {
      BardXCCharacter* data = st->characters - 1;
...
void BardXCInteger64__create__Integer_Integer( BardVM* vm )
{
  ...
  sp->integer64 = ((((BardXCInt64)(sp[-1].integer)) << 32LL) | (((BardXCInt64)(sp[-2].integer)) & 0xffffFFFFLL));
}

void BardXCInteger64__high( BardVM* vm )
{
  vm->sp->integer = (BardXCInteger) (vm->sp->integer64 >> 32);
}

void BardXCInteger64__low( BardVM* vm )
{
  vm->sp->integer = (BardXCInteger) vm->sp->integer64;
}
...
void BardXCInteger64__to_Real( BardVM* vm )
{
  vm->sp->real = (BardXCReal) vm->sp->integer64;
}
...
void BardMath__atan2__Real_Real( BardVM* vm )
{
  BardXCReal x = BardVM_pop_real( vm );
  BardXCReal y = BardVM_pop_real( vm );
  vm->sp->real = atan2( y, x );
}

void BardMath__cos__Real( BardVM* vm )
{
  BardXCReal value = BardVM_pop_real( vm );
...
void BardNativeFileWriter__write__ByteList_Integer_Integer( BardVM* vm )
{
  ...
  {
    BardXCByte* src = BardList_data( list )->byte_data + offset;
...
void BardRandom__next_state__Real( BardVM* vm )
{
  BardXCInt64 state = (BardXCInt64) BardVM_pop_real( vm );
  BardVM_pop_discard( vm );  // context object
  BardVM_push_real( vm, (BardXCReal) ((state * 0xDEECe66dLL + 11LL) & 0xFFFFffffFFFFLL) );
}
...
void BardString__create__ByteList( BardVM* vm )
{
  ...
  if ( !list ) list = BardXCCharacterList_create( vm, 0 );

  count = BardList_count( list );
  BardArray* array = BardList_data(list);
  BardXCByte* src = array->byte_data - 1;
  BardString* st = BardString_create_with_size( vm, count );
  BardXCCharacter* dest = st->characters - 1;
  while (--count >= 0)
  {
    *(++dest) = *(++src);
  }
...
void BardString__compare_to__String( BardVM* vm )
{
  ...
  else
  {
    ...
    BardXCCharacter* a_data = a->characters - 1;
    BardXCCharacter* b_data = b->characters - 1;
    while (--count >=0 )
    {
      if (*(++a_data) != *(++b_data))
      {
