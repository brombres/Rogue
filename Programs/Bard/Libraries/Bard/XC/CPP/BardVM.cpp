...
{
  BardVM* vm = (BardVM*) BARD_ALLOCATE( sizeof(BardVM) );
  return BardVM_init( vm );
}

BardVM* BardVM_destroy( BardVM* vm )
{
  if (vm) BARD_FREE( BardVM_release(vm) );
  return NULL;
}
...
BardVM* BardVM_register_native_method( BardVM* vm, const char* type_context_name, const char* signature, BardVMNativeMethod fn_ptr )
{
  ...
  if ( !info )
  {
    info = (BardVMNativeMethodInfo*) BARD_ALLOCATE( sizeof(BardVMNativeMethodInfo) );
...
void BardVM_throw_error( BardVM* vm, int code, const char* message )
{
  ...
  if (message)
  {
    vm->error_message = (char*) BARD_ALLOCATE( strlen(message)+1 );
...
void BardVM_dispatch_messages( BardVM* vm )
{
  if (vm->message_writer)
  {
    BardXCByte* message_characters = BardVMMessageWriter_to_c_string( vm->message_writer );
    ...
    if (message_buffer)
    {
      ...
      if (message_buffer)
      {
        ...
        BardXCCharacter*     data = BardList_data( message_buffer )->character_data;
...
void BardVM_dispatch_events( BardVM* vm )
{
  if (vm->type_EventQueue)
  {
    ...
    memcpy( character_buffer->character_data, vm->event_queue->character_data, vm->event_queue->character_count * sizeof(BardXCCharacter) );
    memcpy( real_buffer->real_data, vm->event_queue->real_data, vm->event_queue->real_count * sizeof(BardXCReal) );
...
{
  if (vm->mm.bytes_allocated_since_gc >= BARD_XC_BYTES_BEFORE_AUTO_GC)
  {
      ...
      case BARD_TRACE_TYPE_STANDARD:
        {
          ...
          while (--count >= 0)
          {
            Bard_collect_reference( (*((BardObject**)(((BardXCByte*)obj) + *(++cur_offset)))), pending_objects );
          }
        }
        continue;
