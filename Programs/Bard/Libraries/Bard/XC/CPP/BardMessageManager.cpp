...
BardVMMessageWriter* BardVMMessageWriter_create()
{
  ...
  BardVMMessageWriter* writer = (BardVMMessageWriter*) BARD_ALLOCATE( sizeof(BardVMMessageWriter) );
  memset( writer, 0, sizeof(BardVMMessageWriter) );
  writer->data = (BardXCByte*) BARD_ALLOCATE( initial_capacity * sizeof(BardXCByte) );
...
void BardVMMessageWriter_ensure_additional_capacity( BardVMMessageWriter* writer, int additional_capacity )
{
  ...
  if (desired_capacity > writer->capacity)
  {
    ...
    {
      BardXCByte* new_data = (BardXCByte*) BARD_ALLOCATE( desired_capacity * sizeof(BardXCByte) );
      memcpy( new_data, writer->data, writer->count * sizeof(BardXCByte) );
...
BardXCByte* BardVMMessageWriter_to_c_string( BardVMMessageWriter* writer )
{
  if ( !writer->closed )
  {
...
BardVMMessageReader* BardVMMessageReader_create()
{
  ...
  BardVMMessageReader* reader = (BardVMMessageReader*) BARD_ALLOCATE( sizeof(BardVMMessageReader) );
  memset( reader, 0, sizeof(BardVMMessageReader) );

  reader->data = (BardXCByte*) BARD_ALLOCATE( initial_capacity * sizeof(BardXCByte) );
...
void BardVMMessageReader_ensure_capacity( BardVMMessageReader* reader, int desired_capacity )
{
  if (desired_capacity > reader->capacity)
  {
    ...
    BARD_FREE( reader->data );
    reader->data = (BardXCByte*) BARD_ALLOCATE( desired_capacity * sizeof(BardXCByte) );
...
char* BardVMMessageReader_read_c_string( BardVMMessageReader* reader, const char* key, char* buffer, int buffer_size )
{
  ...
  if (BardVMMessageReader_find_key(reader,key))
  {
    ...
    if ( !buffer )
    {
      ...
      buffer = (char*) BARD_ALLOCATE( buffer_size );
    }
    ...
    if (is_literal_string)
    {
      ...
      BardXCByte* dest = (BardXCByte*) (buffer - 1);
      BardXCByte* src  = (reader->data + i1) - 1;
...
BardXCInteger BardVMMessageReader_read_integer( BardVMMessageReader* reader, const char* key )
{
  return (BardXCInteger) BardVMMessageReader_read_real( reader, key );
}

BardXCLogical BardVMMessageReader_read_logical( BardVMMessageReader* reader, const char* key )
{
  return BardVMMessageReader_read_real( reader, key ) ? 1 : 0;
}

BardXCReal BardVMMessageReader_read_real( BardVMMessageReader* reader, const char* key )
{
  int i1, i2;
  double result = 0.0;
...
void BardVMMessageReader_set_character_data( BardVMMessageReader* reader, BardXCCharacter* data, int data_count )
{
  BardXCByte* dest;
  int i;
  ...
  reader->count = data_count;
  while (--data_count >= 0) *(++dest) = (BardXCByte) *(++data);
  *(++dest) = 0;
...
{
  BardMessageListener* listener = (BardMessageListener*) BARD_ALLOCATE( sizeof(BardMessageListener) );
