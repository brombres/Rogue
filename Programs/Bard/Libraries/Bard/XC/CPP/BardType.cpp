...
void BardType_destroy( BardType* type )
{
  ...
  {
    BARD_FREE( type->reference_setting_offsets );
  ...
  {
    BARD_FREE( type->reference_property_offsets );
...
void BardType_set_reference_sizes( BardType* type )
{
  ...
  {
    if (type == vm->type_Real)           type->reference_size = sizeof(BardXCReal);
    else if (type == vm->type_Integer)   type->reference_size = sizeof(BardXCInteger);
    else if (type == vm->type_Character) type->reference_size = sizeof(BardXCCharacter);
...
void BardType_organize( BardType* type )
{
  ...
  if (count)
  {
    ...
    type->reference_setting_offsets = (int*) BARD_ALLOCATE( type->reference_setting_count * sizeof(int) );
    dest_i = 0;
    for (i=0; i<count; ++i)
    {
  ...
  if (count)
  {
    ...
    for (i=0; i<count; ++i)
    {
      if (BardType_is_reference( type->properties[i].type )) ++type->reference_property_count;
    }

    type->reference_property_offsets = (int*) BARD_ALLOCATE( type->reference_property_count * sizeof(int) );
    dest_i = 0;
    for (i=0; i<count; ++i)
    {
      BardProperty* property = &type->properties[i];
      if (BardType_is_reference(property->type)) type->reference_property_offsets[dest_i++] = property->offset;
    }
  }
