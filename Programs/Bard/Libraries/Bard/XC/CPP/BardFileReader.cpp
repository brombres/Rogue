...
{
  return BardFileReader_init_with_filename( (BardFileReader*) BARD_ALLOCATE(sizeof(BardFileReader)), filename );
}

BardFileReader* BardFileReader_create_with_data( const char* filename, char* data, int count, int free_data_on_close )
{
  return BardFileReader_init_with_data( (BardFileReader*) BARD_ALLOCATE(sizeof(BardFileReader)), filename, data, count, free_data_on_close );
}

void BardFileReader_destroy( BardFileReader* reader )
{
  BARD_FREE( BardFileReader_close(reader) );
}
...
BardFileReader* BardFileReader_init_with_filename( BardFileReader *reader, const char* filename )
{
  ...
  char* buffer = (char*) BARD_ALLOCATE( size );
...
char* BardFileReader_read_id( BardFileReader* reader )
{
  ...
  char* buffer = (char*) BARD_ALLOCATE( count+1 );
