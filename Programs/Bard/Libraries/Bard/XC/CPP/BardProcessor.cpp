...
{
  return (BardProcessor*) Bard_free( program );
}
...
void BardProcessor_execute( BardVM* vm )
{
  ...
  BardXCInteger*    ip = current_frame->ip;
...
if (sp->object) printf( "Returning object type %s\n", sp->object->type->name );
      ...
      case BARD_VM_OP_CONVERT_INTEGER_TO_REAL:
        sp->real = (BardXCReal) sp->integer;
        continue;

      case BARD_VM_OP_CONVERT_REAL_TO_INTEGER:
        sp->integer = (BardXCInteger) sp->real;
        continue;
...
if (trace)
{
  ...
  if (sp->object) printf( "%s\n", sp->object->type->name );
      ...
      case BARD_VM_OP_PUSH_LITERAL_REAL:
        (--sp)->real = (BardXCReal)vm->literal_reals[ *(++ip) ];
        continue;
      ...
      case BARD_VM_OP_PUSH_LITERAL_INTEGER_N_AS_REAL:
        (--sp)->real = (BardXCReal) BARD_VM_OP_ARG_N(op);
        continue;
...
printf( "  TYPE: %s\n", sp->object->type->name );
      ...
      case BARD_VM_OP_INCREMENT_PROPERTY_REAL_N:
        ++ *((BardXCReal*)(((char*)((sp++)->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_INCREMENT_PROPERTY_INTEGER_N:
        ++ *((BardXCInteger*)(((char*)((sp++)->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_INCREMENT_THIS_PROPERTY_REAL_N:
        ++ *((BardXCReal*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_INCREMENT_THIS_PROPERTY_INTEGER_N:
        ++ *((BardXCInteger*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;
      ...
      case BARD_VM_OP_READ_SETTING_REAL_OFFSET_N:
        (--sp)->real = *((BardXCReal*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))); 
        continue;

      case BARD_VM_OP_READ_SETTING_INTEGER_OFFSET_N:
        (--sp)->integer = *((BardXCInteger*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))); 
        continue;

      case BARD_VM_OP_READ_SETTING_CHARACTER_OFFSET_N:
        (--sp)->integer = *((BardXCCharacter*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))); 
        continue;
      ...
      case BARD_VM_OP_READ_PROPERTY_REAL_OFFSET_N:
        if (!sp->object) goto throw_null_reference_error;
        sp->real = *((BardXCReal*)(((char*)(sp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_READ_PROPERTY_INTEGER_OFFSET_N:
        if (!sp->object) goto throw_null_reference_error;
        sp->integer = *((BardXCInteger*)(((char*)(sp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_READ_PROPERTY_CHARACTER_OFFSET_N:
        if (!sp->object) goto throw_null_reference_error;
        sp->integer = *((BardXCCharacter*)(((char*)(sp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;
      ...
      case BARD_VM_OP_READ_THIS_PROPERTY_REAL_OFFSET_N:
        (--sp)->real = *((BardXCReal*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_READ_THIS_PROPERTY_INTEGER_OFFSET_N:
        (--sp)->integer = *((BardXCInteger*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;

      case BARD_VM_OP_READ_THIS_PROPERTY_CHARACTER_OFFSET_N:
        (--sp)->integer = *((BardXCCharacter*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op)));
        continue;
...
//if (trace)printf("  Slot count %d\n", slot_count );
      ...
      case BARD_VM_OP_WRITE_SETTING_REAL_OFFSET_N:
        *((BardXCReal*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))) = (sp++)->real;
        continue;

      case BARD_VM_OP_WRITE_SETTING_INTEGER_OFFSET_N:
        *((BardXCInteger*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))) = (sp++)->integer;
        continue;

      case BARD_VM_OP_WRITE_SETTING_CHARACTER_OFFSET_N:
        *((BardXCCharacter*)((vm->types[ *(++ip) ]->settings_data) + BARD_VM_OP_ARG_N(op))) = (sp++)->integer;
        continue;
      ...
      case BARD_VM_OP_WRITE_PROPERTY_REAL_OFFSET_N:
        if ( !sp[1].object ) goto throw_null_reference_error;
        *((BardXCReal*)(((char*)(sp[1].object)) + BARD_VM_OP_ARG_N(op))) = sp->real;
        sp += 2;
        continue;

      case BARD_VM_OP_WRITE_PROPERTY_INTEGER_OFFSET_N:
        if ( !sp[1].object ) goto throw_null_reference_error;
        *((BardXCInteger*)(((char*)(sp[1].object)) + BARD_VM_OP_ARG_N(op))) = sp->integer;
        sp += 2;
        continue;

      case BARD_VM_OP_WRITE_PROPERTY_CHARACTER_OFFSET_N:
        if ( !sp[1].object ) goto throw_null_reference_error;
        *((BardXCCharacter*)(((char*)(sp[1].object)) + BARD_VM_OP_ARG_N(op))) = sp->integer;
        sp += 2;
        continue;
      ...
      case BARD_VM_OP_WRITE_THIS_PROPERTY_REAL_OFFSET_N:
        *((BardXCReal*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op))) = (sp++)->real;
        continue;

      case BARD_VM_OP_WRITE_THIS_PROPERTY_INTEGER_OFFSET_N:
        *((BardXCInteger*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op))) = (sp++)->integer;
        continue;

      case BARD_VM_OP_WRITE_THIS_PROPERTY_CHARACTER_OFFSET_N:
        *((BardXCCharacter*)(((char*)(fp->object)) + BARD_VM_OP_ARG_N(op))) = (BardXCCharacter) ((sp++)->integer);
        continue;
...
//if (trace)printf("  Slot count %d\n", slot_count );
      ...
      case BARD_VM_OP_READ_COMPOUND_REAL_OFFSET_A_DELTA_SP_B:
        {
          BardXCReal result = sp[BARD_VM_OP_ARG_A(op)].real;
          sp += BARD_VM_OP_ARG_B( op );
          sp->real = result;
        }
        continue;
      ...
      case BARD_VM_OP_READ_COMPOUND_INTEGER_OFFSET_A_DELTA_SP_B:
        {
          BardXCInteger result = sp[BARD_VM_OP_ARG_A(op)].integer;
          sp += BARD_VM_OP_ARG_B( op );
          sp->integer = result;
        }
        continue;
...
printf( "  parameter slot count:%d, local slot count:%d\n", m->parameter_slot_count, m->local_slot_count );
      ...
      case BARD_VM_OP_DIVIDE_INTEGER:
        {
          ...
          ++sp;
          sp->integer = (BardXCInteger)(((BardXCReal)sp->integer) / (BardXCReal)sp[-1].integer);
        }
        continue;
      ...
      case BARD_VM_OP_POWER_INTEGER:
        ++sp;
        sp->integer = (BardXCInteger) pow(sp->integer, sp[-1].integer);
        continue;
      ...
      case BARD_VM_OP_WRITE_ARRAY_ELEMENT_CHARACTER:
        {
          int index = sp[1].integer;
          if ( !sp[2].object ) goto throw_null_reference_error;
          ((BardArray*)sp[2].object)->character_data[ index ] = (BardXCCharacter) sp[0].integer;
          sp += 2;
        }
        continue;
      ...
      case BARD_VM_OP_WRITE_ARRAY_ELEMENT_BYTE:
        {
          int index = sp[1].integer;
          if ( !sp[2].object ) goto throw_null_reference_error;
          ((BardArray*)sp[2].object)->byte_data[ index ] = (BardXCByte) sp[0].integer;
          sp += 2;
        }
        continue;
      ...
      case BARD_VM_OP_SHR_INTEGER:
        {
          ...
          BardXCInteger a = (++sp)->integer;
          BardXCInteger b = sp[-1].integer;
          if(b == 0) continue;
