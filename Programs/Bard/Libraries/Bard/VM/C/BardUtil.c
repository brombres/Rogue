...
char* BardVMCString_duplicate( const char* st )
{
  ...
  buffer = CROM_SYSTEM_MALLOC( count + 1 );
...
{
  void* result = CROM_SYSTEM_MALLOC( size );
...
{
  int count;
  BardHashTable* table = (BardHashTable*) CROM_SYSTEM_MALLOC( sizeof(BardHashTable) );
  ...
  table->bins = (BardHashTableEntry**) CROM_SYSTEM_MALLOC( bin_count * sizeof(BardHashTableEntry*) );
  ...
  if (table)
  {
    ...
    for (i=0; i<=table->bin_mask; ++i)
    {
      ...
      while (cur)
      {
        ...
        CROM_SYSTEM_FREE( cur->key );
        CROM_SYSTEM_FREE( cur );
        cur = next;
      }
    }
    CROM_SYSTEM_FREE( table->bins );
    CROM_SYSTEM_FREE( table );
  }
  return NULL;
}
...
void BardHashTable_set( BardHashTable* table, const char* key, void* value )
{
  ...
  // New entry
  cur = (BardHashTableEntry*) CROM_SYSTEM_MALLOC( sizeof(BardHashTableEntry) );
...
{
  return BardIntList_init( CROM_SYSTEM_MALLOC( sizeof(BardIntList) ) );
}
...
void BardIntList_ensure_capacity( BardIntList* list, int min_capacity )
{
  ...
  if (list->data)
  {
    int* new_data = CROM_SYSTEM_MALLOC( sizeof(int) * min_capacity );
  ...
  {
    list->data = CROM_SYSTEM_MALLOC( sizeof(int) * min_capacity );
  }
...
{
  return BardPointerList_init( CROM_SYSTEM_MALLOC( sizeof(BardPointerList) ) );
}
...
BardPointerList* BardPointerList_free_elements( BardPointerList* list )
{
  while (list->count > 0)
  {
    CROM_SYSTEM_FREE( list->data[ --list->count ] );
  }
  return list;
}
...
void BardPointerList_ensure_capacity( BardPointerList* list, int min_capacity )
{
  ...
  if (list->data)
  {
    void** new_data = CROM_SYSTEM_MALLOC( sizeof(void*) * min_capacity );
  ...
  {
    list->data = CROM_SYSTEM_MALLOC( sizeof(void*) * min_capacity );
  }
  list->capacity = min_capacity;
}
...
{
  int count;
  BardResourceIDTable* table = (BardResourceIDTable*) CROM_SYSTEM_MALLOC( sizeof(BardResourceIDTable) );
  ...
  table->bins = (BardResourceIDTableEntry**) CROM_SYSTEM_MALLOC( bin_count * sizeof(BardResourceIDTableEntry*) );
...
void BardResourceIDTable_set( BardResourceIDTable* table, void* key, int value )
{
  ...
  // New entry
  cur = (BardResourceIDTableEntry*) CROM_SYSTEM_MALLOC( sizeof(BardResourceIDTableEntry) );
  cur->key = key;
  cur->value = value;
  cur->next_entry = first;
  table->bins[slot] = cur;
...
{
  BardResourceBank* bank = (BardResourceBank*) CROM_SYSTEM_MALLOC( sizeof(BardResourceBank) );
