...
{
  BardMethod* m = (BardMethod*) CROM_SYSTEM_MALLOC( sizeof(BardMethod) );
  ...
  if (local_count)
  {
    m->local_types = (BardType**) CROM_SYSTEM_MALLOC( sizeof(BardType*) * local_count );
  }
...
void BardMethod_destroy( BardMethod* m )
{
  if (m->exception_handlers)
  {
    ...
    for (i=0; i<m->exception_handler_count; ++i)
    {
      ...
      if (handler->catches) CROM_SYSTEM_FREE( handler->catches );
    }
    CROM_SYSTEM_FREE( m->exception_handlers );
  ...
  if (m->tags)
  {
    CROM_SYSTEM_FREE( m->tags );
    m->tags = NULL;
  }

  if (m->local_types)
  {
    CROM_SYSTEM_FREE( m->local_types );
    m->local_types = NULL;
  }

  if (m->signature)
  {
    CROM_SYSTEM_FREE( m->signature );
    m->signature = NULL;
  }
...
void BardMethod_create_signature_if_necessary( BardMethod* m )
{
  ...
  m->signature = CROM_SYSTEM_MALLOC( sig_len+1 );
