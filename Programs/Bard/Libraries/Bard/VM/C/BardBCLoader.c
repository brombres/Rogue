...
int BardBCLoader_load_bc_from_reader( BardVM* vm, BardFileReader* reader )
{
  BARD_TRY(vm)
    ...
    // Load code chunk
    {
      ...
      vm->code = (BardInteger*) CROM_SYSTEM_MALLOC( sizeof(BardInteger) * vm->code_size );
    ...
    // Read identifiers
    {
      ...
      vm->identifiers = (char**) CROM_SYSTEM_MALLOC( sizeof(char*) * vm->identifier_count );
    ...
    // Read type declarations
    {
      ...
      vm->types = (BardType**) CROM_SYSTEM_MALLOC( sizeof(BardType*) * vm->type_count );
      ...
      for (i=0; i<vm->type_count; ++i)
      {
        ...
        {
          int t;
          type->tags = (char**) CROM_SYSTEM_MALLOC( sizeof(char*) * type->tag_count );
        ...
        {
          int b;
          type->base_types = (BardType**) CROM_SYSTEM_MALLOC( sizeof(BardType*) * type->base_type_count );
        ...
        {
          int s;
          type->settings = (BardProperty*) CROM_SYSTEM_MALLOC( sizeof(BardProperty) * type->settings_count );
        ...
        {
          int p;
          type->properties = (BardProperty*) CROM_SYSTEM_MALLOC( sizeof(BardProperty) * type->property_count );
    ...
    // Read method definitions
    {
      ...
      vm->methods = (BardMethod**) CROM_SYSTEM_MALLOC( sizeof(BardMethod*) * vm->method_count );
      for (i=0; i<vm->method_count; ++i)
      {
        ...
        {
          int t;
          m->tags = (char**) CROM_SYSTEM_MALLOC( sizeof(char*) * m->tag_count );
        ...
        if (m->exception_handler_count)
        {
          ...
          m->exception_handlers = (BardVMExceptionHandlerInfo*) CROM_SYSTEM_MALLOC( size );
          for (e=0; e<m->exception_handler_count; ++e)
          {
            ...
            {
              int c;
              info->catches = (BardVMExceptionHandlerCatchInfo*) CROM_SYSTEM_MALLOC( sizeof(BardVMExceptionHandlerCatchInfo) * info->catch_count );
    ...
    // Read per-type method tables
    {
      ...
      for (i=0; i<type_count; ++i)
      {
        ...
        {
          int m;
          type->methods = (BardMethod**) CROM_SYSTEM_MALLOC( sizeof(BardMethod*) * type->method_count );
        ...
        // Load aspect call tables
        {
          ...
          if (type->aspect_count > 0)
          {
            ...
            type->aspect_types = (BardType**) CROM_SYSTEM_MALLOC( sizeof(BardType*) * type->aspect_count );
            type->aspect_call_tables = (BardMethod***) CROM_SYSTEM_MALLOC( sizeof(BardMethod**) * type->aspect_count );
            ...
            for (a=0; a<type->aspect_count; ++a)
            {
              ...
              type->aspect_call_tables[a] = (BardMethod**) CROM_SYSTEM_MALLOC( sizeof(BardMethod*) * m_count );
    ...
    // Load Real literals
    {
      ...
      {
        vm->literal_reals = (BardReal*)CROM_SYSTEM_MALLOC( sizeof(BardReal) * vm->literal_real_count);

        for(i = 0; i < vm->literal_real_count; ++i)
        {
          BardInteger upper32_of_real = (BardInteger)BardFileReader_read_integer( reader );
          BardInteger lower32_of_real = (BardInteger)BardFileReader_read_integer( reader );
    ...
    // Load filenames used
    {
      ...
      vm->filenames = (char**) CROM_SYSTEM_MALLOC( sizeof(char*) * vm->filename_count );
    ...
    // Load line info
    {
      ...
      if (vm->line_info_count)
      {
        ...
        vm->line_info = (unsigned char*) CROM_SYSTEM_MALLOC( vm->line_info_count );
        for (i=0; i<count; ++i)
        {
    ...
    // Load string literals
    {
      ...
      {
        vm->literal_strings = (BardObject**) CROM_SYSTEM_MALLOC( sizeof(BardObject*) * vm->literal_string_count );
